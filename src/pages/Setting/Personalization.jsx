...
const Personalization = () => {
  ...
  const fetchProfile = async () => {
    let token = getToken();
    try {
      const res = await axios.get("https://cashpay.co.id:2356/api/auth/profile", {
      ...
      if (err.response?.status === 401) {
        ...
        const retryRes = await axios.get("https://cashpay.co.id:2356/api/auth/profile", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await axios.put(
        `https://cashpay.co.id:2356/api/auth/profile/update?id=${data.id}`,
        {
