...
const List = () => {
  ...
  const loadRoles = useCallback(async () => {
    try {
      ...
      const response = await fetchRoles(paginationData);
  ...
  // ➕ Tambah produk baru
  const handleAddRole = () => {
    if (!newRole.product_name || !newRole.price) {
    ...
    const newItem = {
      id: "P" + (data.length + 1),
      product_name: newRole.product_name,
      price: parseInt(newRole.price),
      stock: newRole.stock || 0,
    ...
    setNewRole({ product_name: "", price: "", stock: "" });
    setShowModal(false);
  };
  ...
  const handleShowDetail = async (row) => {
    try {
      const product = await fetchRoleById(row.id);
      if (product) {
  ...
  return (
    ...
    >
      ...
      {showDetailModal && (
        <RoleDetailModal product={selectedRole} onClose={() => setShowDetailModal(false)} />
      )}
